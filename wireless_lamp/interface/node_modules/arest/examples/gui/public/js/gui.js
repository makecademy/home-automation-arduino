...
$(document).ready ( function () {
  ...
  $(document).on("click", ".pin", function(e){
    ...
    if (pinsCommand[e.target.id] == "digitalWrite") { 
        ...
        $.get( "/send", {type: comm_type, target: comm_target, command: "/digital/" + e.target.id + "/" + pinsStatus[e.target.id]} );
    }
    ...
    if (pinsCommand[e.target.id] == "analogWrite") { 
        ...
        // Send command
        $.get( "/send", {type: comm_type, target: comm_target, command: "/analog/" + e.target.id + "/" + value} );
    }
    ...
    if (pinsCommand[e.target.id] == "digitalRead") {
      ...
      // Get data
      $.get( "/send", {type: comm_type, target: comm_target, command: "/digital/" + e.target.id + "/r"}, function(data) {
        obj = JSON.parse(data);
    ...
    if (pinsCommand[e.target.id] == "analogRead") {
      ...
      // Get data & display
      $.get( "/send", {type: comm_type, target: comm_target, command: "/analog/" + e.target.id + "/r"}, function(data) {
        obj = JSON.parse(data);
  ...
  $(document).on("click", ".selectorElement", function(e){
    ...
    if (selected_command == "digitalWrite") {
      ...
      $.get( "/send", {type: comm_type, target: comm_target, command: "/mode/" + selected_pin + "/o"} );
    }

    if (selected_command == "digitalRead") {
      $("#" + selected_pin).css("border-color","green");
      $("#" + selected_pin).css("background-color","white");
      $.get( "/send", {type: comm_type, target: comm_target, command: "/mode/" + selected_pin + "/i"} );
    }
